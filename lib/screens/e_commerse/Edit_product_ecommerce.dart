...
class EditProductEcommerse extends StatefulWidget {
  ...
  EditProductEcommerse(
      ...
      this.long);
  @override
  _EditProductEcommerseState createState() => _EditProductEcommerseState(
      this.name,
      this.image,
      this.photo,
      this.product,
                      ...
                      if (_titleController.text.trim() != "" &&
                        ...
                        EasyLoading.show(
                            status: appState.getlocal == "ar"
                                ...
                                : "Editing The Product");
                        try {
                          // "image" : "gs://jeraanapp-7f69a.appspot.com/main${image.path}"
                          // List<String> listId = [];
                          // List<double> listL = [];
                          // List<double> listG = [];
                          ...
                          if (_myDoc.size > 0) {
                            for (var i = 0; i < _myDoc.size; i++) {
                              print(
                                  "${_myDoc.docs[i]['index']} : ${_myDoc.size - 1}");
                              if (_myDoc.docs[i]["index"] ==
                                  "${_myDoc.size - 1}") {
                                ...
                                if (l.length <= 200) {
                                  ...
                                  // listPhotos.add("gs://jeraanapp-7f69a.appspot.com/main${image.path}");
                                  ...
                                  ref.update({
                                    "data":
                                        FieldValue.arrayRemove([ecommerceMap])
                                  });
                                  ecommerceServicesUpdate(
                                          appState.getimage,
                                          appState.getname,
                                          appState.getlat,
                                          appState.getlong,
                            ...
                            ref.update({
                              "data": FieldValue.arrayRemove([ecommerceMap])
                            });
                            ...
                            ecommerceServicesSet(
                                    ...
                                    DateTime.now().toString(),
                                    FirebaseAuth.instance.currentUser.uid,
                                    _whatsController.text.trim().toString(),
                                    photo,
                          ...
                          EasyLoading.showError(
                              appState.getlocal == "ar"
                                  ? "عفوا حدث خطأ ما"
                                  : "sorry you request not complited",
        ...
        return AlertDialog(
          ...
          content: Row(
            children: [
              ...
              Text(
                ...
                style: TextStyle(
                    fontSize: appState.getlocal == "ar" ? 19 : 15,
          ...
          actions: [
            TextButton(
              child: Text(
                appState.getlocal == "ar"
                    ? 'الرجوع الي القائمة السابقة'
                    : "Return to the previous menu",
                style: TextStyle(
                    fontSize: appState.getlocal == "ar" ? 15 : 13.5,
                    color: Colors.black),
                textDirection: TextDirection.rtl,
              ),
              onPressed: () {
                ...
                Navigator.pushReplacement(context,
                    MaterialPageRoute(builder: (context) => ECommerseHome()));
              },
            ),
            TextButton(
              child: Text(
                appState.getlocal == "ar"
                    ? 'الذهاب الي القائمة الرئيسيه'
                    : 'Go to main menu',
                style: TextStyle(
                    fontSize: appState.getlocal == "ar" ? 15 : 13.5,
                    color: Colors.black),
