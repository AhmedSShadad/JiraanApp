...
class _HelpHomeState extends State<HelpHome> {
                ...
                child: TextButton(
                    onPressed: () {
                      Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) => AddProductHelp()));
                    },
                    child: Text(
                      appState.getlocal == "ar" ? 'إضافة منتج' : "Add Product",
                      style: TextStyle(
                          fontSize: 20,
                          ...
                          children: [
                            IconButton(
                                onPressed: () {
                                  ...
                                  ref.update({
                                    "data":
                                        FieldValue.arrayRemove([beforeEventMap])
                                  }).then((value) {
                            ...
                            IconButton(
                                onPressed: () {
                                  Navigator.push(
                                      context,
                                      MaterialPageRoute(
                                          builder: (context) => EditProductHelp(
                                              name,
                                              image,
                                              photo,
                                              product,
  ...
  Future getData() async {
    try {
      ...
      double myDistance = appState.getnearest == null
          ? 0
          : double.parse(appState.getnearest) * 1000;
      QuerySnapshot _myDoc =
          await FirebaseFirestore.instance.collection('Est3arat').get();
      if (_myDoc.size > 0) {
