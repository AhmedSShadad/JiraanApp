...
class _T3arfHomeState extends State<T3arfHome> {
    ...
    _scrollController.addListener(() {
      if (_scrollController.position.extentAfter == 0) {
        if (myIndex.trim() != "" && myIndex != "0") {
          EasyLoading.show(
              status: appState.getlocal == "ar"
                  ? "جاري تحميل المزيد"
                  : "Loading ..");
          GetNextData();
        } else {
          EasyLoading.showInfo(
              appState.getlocal == "ar" ? "لا يوجد المزيد" : "No more posts",
  ...
  Future GetData() async {
    ...
    QuerySnapshot _myDoc =
        await FirebaseFirestore.instance.collection('T3arf').get();
    if (_myDoc.size > 0) {
      FirebaseFirestore.instance
          .collection("T3arf")
          .where("index", isEqualTo: (_myDoc.size - 1).toString())
          .get()
          .then((value) {
        ...
        for (var i = mainListData.length - 1; i >= 0; i--) {
          double distance = geodesy
              .distanceBetweenTwoGeoPoints(
                  l1, LatLng(mainListData[i]["L"], mainListData[i]["G"]))
              .toDouble();
      ...
      EasyLoading.showInfo(
          appState.getlocal == "ar" ? "لا توجد منشورات" : "No posts founded",
  ...
  Future GetNextData() async {
    try {
      ...
      FirebaseFirestore.instance
          .collection("T3arf")
          .where("index", isEqualTo: (int.parse(myIndex) - 1).toString())
          .get()
          .then((value) {
        // mainlistimage = List.from(value.docs[0]["listImage"]);
        // mainlistage = List.from(value.docs[0]["listAge"]);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
                  ...
                  child: TextButton(
                      onPressed: () async {
                        Navigator.push(
                            context,
                            MaterialPageRoute(
                                builder: (context) => AddPostT3arf()));
                      },
                      child: Text(
                        appState.getlocal == "ar" ? 'إضافة منشور' : "Add Post",
                        style: TextStyle(
                            fontSize: 20,
        ...
        appBar: AppBar(
          title: Text(appState.getlocal == "ar"
              ? 'تعارف الجيران '
              : "know the neighbors"),
          centerTitle: true,
        ),
              ...
              child: Column(
                ...
                children: [
                  Container(
                    ...
                    height: 100,
                    child: Text(
                      appState.getlocal == "ar"
                          ? 'رجاءا من مسئول التطبيق : لا تستخدم هذه الخاصية في إزعاج غيرك من الجيران وراقب الله في افعالك.'
                          : 'Please from the application administrator: Do not use this feature to annoy other neighbors and watch God in your actions.',
        ...
        children: [
          GestureDetector(
            onTap: () {
              Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (context) => ViewUsersScreen("$id")));
            },
            child: Row(
                        ...
                        children: [
                          IconButton(
                              onPressed: () {
                                ...
                                ref.update({
                                  "data":
                                      FieldValue.arrayRemove([beforeT3arfMap])
                                }).then((value) {
                                  Navigator.pushReplacement(
                                      context,
                                      MaterialPageRoute(
                                          builder: (context) => T3arfHome()));
                                });
                              },
                              icon: Icon(
                                Icons.delete,
          ...
          Row(
            ...
            children: [
              Text(
                different == 0
                    ? (appState.getlocal == "ar" ? "الآن" : "Now")
                    : (different > 0 && different < 60)
                        ? (appState.getlocal == "ar"
                            ? "قبل $different دقيقة"
                            : "from $different minutes")
                        : (different >= 60 && hour < 24)
                            ? (appState.getlocal == "ar"
                                ? "قبل $hour ساعة"
                                : "from $hour hour")
                            : (appState.getlocal == "ar"
                ...
                onPressed: () {
                  FlutterClipboard.copy(phone).then((value) =>
                      EasyLoading.showSuccess(
                          (appState.getlocal == "ar" ? "تم النسخ" : "Copied"),
                          duration: Duration(milliseconds: 600)));
                },
