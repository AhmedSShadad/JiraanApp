...
class _MyAppState extends State<MyApp> {
  ...
  getLocation() async {
    ...
    try {
      ...
      if (!_serviceEnabled) {
        _serviceEnabled =
            await location.requestService().then((value) => getLocation());
        if (!_serviceEnabled) {
          _serviceEnabled =
              await location.requestService().then((value) => getLocation());
        }
  ...
  setlocation(double lat, double long) async {
    try {
      ...
      String address = addresses == null ? "" : addresses.streetAddress;
      print("$address Shadad");
      await FirebaseFirestore.instance
        ...
        EasyLoading.showSuccess(
            appState.getlocal == "ar" ? "تم تجهيز التطبيق" : "Ready to use",
            duration: Duration(milliseconds: 400));
      });
    } catch (e) {
      print("{$e.toString()} Shadad");
    }
  }
}
